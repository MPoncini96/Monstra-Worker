...
# Rank weights for Alpha1
RANK_WEIGHTS = np.array([0.40, 0.30, 0.20, 0.10], dtype=float)
MAX_RANKED_HOLDINGS = len(RANK_WEIGHTS)
...
def clamp_top_n(top_n: int | None) -> int:
    if top_n is None:
        return MAX_RANKED_HOLDINGS
    return max(1, min(int(top_n), MAX_RANKED_HOLDINGS))
...
def fetch_active_alpha1_bots() -> list[Alpha1BotConfig]:
    ...
    if extended:
        ...
        for row in rows:
            bot_id, name, universe_raw, top_n, lookback_days, cash_equiv = row
            configs.append(
                Alpha1BotConfig(
                    bot_id=str(bot_id),
                    name=str(name),
...
def choose_risk_on_holdings(ranked_trailing: pd.Series, top_n: int) -> tuple[list[str], np.ndarray]:
    if ranked_trailing.empty:
    ...
    weights = RANK_WEIGHTS[:len(selected)].copy()
...
def choose_holdings_for_day(
    ...
    selected, weights = choose_risk_on_holdings(ranked_trailing, effective_top_n)
...
def build_holdings_payload(
    ...
    payload["_meta"] = {
        ...
        "configured_top_n": int(clamp_top_n(bot.top_n)),
        "selected_holdings": int(selected_count),
        "weight_model": "40-30-20-10",
...
def backfill_single_bot(bot: Alpha1BotConfig) -> dict[str, Any]:
    logger.info(
        "Starting bot=%s name=%s universe=%d top_n=%s lookback=%d cash=%s benchmark=%s",
        bot.bot_id,
        bot.name,
        len(bot.universe),
        bot.top_n,
    ...
    effective_top_n = clamp_top_n(bot.top_n)
