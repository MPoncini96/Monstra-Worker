...
def _clamp_top_n(top_n: int | None) -> int:
    ...
    return max(1, min(int(top_n), len(DEFAULT_WEIGHTS)))
...
def _build_live_config(bot_id: str, use_db_config: bool = True) -> tuple[Alpha1Config, str | None]:
    ...
    config.top_n = _clamp_top_n(config.top_n)
...
def _choose_risk_on_holdings(ranked_trailing: pd.Series, top_n: int) -> tuple[list[str], np.ndarray]:
    if ranked_trailing.empty:
        return [], np.array([], dtype=float)

    ranked = ranked_trailing.sort_values(ascending=False)
    selected = ranked.head(top_n).index.tolist()
    weights = DEFAULT_WEIGHTS[:len(selected)].copy()
...
def _choose_holdings_for_day(
    ...
    selected, weights = _choose_risk_on_holdings(ranked_trailing, int(config.top_n or DEFAULT_TOP_N))
...
def run_alpha1(
    ...
    if top_n is not None:
        config.top_n = _clamp_top_n(top_n)
