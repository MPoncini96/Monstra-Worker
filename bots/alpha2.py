...
def pick_best_sector(trailing_ret_row: pd.Series, proxies: dict, bench: str) -> str | None:
    bench_ret = trailing_ret_row.get(bench, np.nan)
    if pd.isna(bench_ret):
        return None

    proxy_rets = {}
    for sector, proxy in proxies.items():
...
def run_alpha2(
    ...
    lookback_days_cfg = DEFAULT_LOOKBACK_DAYS
    history_period_cfg = DEFAULT_HISTORY_PERIOD
    rebalance_freq_cfg = DEFAULT_REBALANCE_FREQ
    stock_bench_mode_cfg = DEFAULT_STOCK_BENCH_MODE
    ...
    if use_db_config:
        try:
            ...
            if config:
                ...
                rank_weights = np.array(config.get("rank_weights", [0.40, 0.30, 0.20, 0.10]), dtype=float)
                lookback_days_cfg = int(config.get("lookback_days", DEFAULT_LOOKBACK_DAYS))
                history_period_cfg = config.get("history_period", DEFAULT_HISTORY_PERIOD)
    ...
    rebalance_freq = rebalance_freq if rebalance_freq is not None else rebalance_freq_cfg
    stock_bench_mode = stock_bench_mode if stock_bench_mode is not None else stock_bench_mode_cfg

    # Build required ticker list: bench + all proxies + all stocks
    all_stocks = sorted({t for lst in stocks.values() for t in lst})
    all_proxies = sorted(set(proxies.values()))
    ...
    best_sector = pick_best_sector(tr_row, proxies, bench)
