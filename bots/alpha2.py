...
def build_price_frame(config: Alpha2Config, start_date: str, end_date: str) -> pd.DataFrame:
    symbols = list(dict.fromkeys(config.proxies.values()))
...
def choose_ranked_holdings(ranked_trailing: pd.Series, rank_weights: np.ndarray) -> tuple[list[str], np.ndarray]:
    if ranked_trailing.empty:
...
def choose_holdings_for_day(
    ...
    selected, weights = choose_ranked_holdings(ranked_trailing, config.rank_weights)

    if not selected and USE_CASH_EQUIVALENT_FALLBACK and config.cash_equivalent:
        return [config.cash_equivalent], np.array([1.0], dtype=float), True, "fallback:no_selected_proxies"
...
def run_alpha2(
    ...
    prices = build_price_frame(config, start_date, end_date)
    ...
    if rebalanced_today:
        ...
        signal = "REBALANCE"
        note = f"As of {asof.date()}: rebalance risk_reason={risk_reason} holdings={', '.join(target_holdings.keys())}"
    else:
        risk_off = False
